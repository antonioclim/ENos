...
#### New Files
- **check_my_submission.sh** - Student self-verification script
  - Checks file existence, extension, size, signature presence
  - Validates filename format
  - Provides clear pass/fail feedback with suggestions
- **examples/sample_submission_demo.cast** - Sample recording for preview
  - Shows what a completed recording looks like
  - Includes demo commands and STOP_homework ending

#### Documentation Improvements
- **Practice Run section** in STUDENT_GUIDE with step-by-step dry run instructions
- **Escalation path** for support (self-help → peer → instructor)
- **Video tutorial placeholder** for future content
- **"From the trenches" anecdotes** — real stories from previous semesters
- **Verification section** explaining how to use check_my_submission.sh

#### Code Quality - Bash Script
- **Full strict mode**: `set -euo pipefail` with `IFS=$'\n\t'`
- **Array-based package installation** — safer than string concatenation
- **`read -r`** for safe input reading throughout
- **`readonly`** declarations for all constants
- **Explicit exit handling** in upload with `set +e` / `set -e`
- Improved variable quoting for edge cases

### Changed

- README_EN.md restructured with verification and examples sections
- FAQ_EN.md expanded with verification section and personal anecdotes
- Version bumped to 1.1.1 in all files
- Sentence structure varied to reduce repetitive patterns
- Added natural colloquialisms ("Honestly, this catches someone every semester")

### Fixed

- CHANGELOG accuracy — now correctly reflects actual script contents
- Strict mode was claimed but not fully implemented; now fixed
- Potential word splitting in package installation
