# 📁 Bash Utilities — SEM05

> **Location:** `SEM05/scripts/bash/`  
...
|--------|---------|----------------|
| `S05_01_setup_seminar.sh` | Install dependencies, create workspace | Yes (first run) |
| `S05_02_interactive_quiz.sh` | CLI quiz with instant feedback | No |
| `S05_03_validator.sh` | Validate homework submissions | No |
...
./S05_01_setup_seminar.sh

# Take a practice quiz
./S05_02_interactive_quiz.sh

# Validate your homework
./S05_03_validator.sh ~/my_homework/
```
...
```bash
./S05_01_setup_seminar.sh [options]
...
- Workspace directories (`~/os_seminar_sem05/`)
...
```bash
./S05_02_interactive_quiz.sh [options]
...
./S05_02_interactive_quiz.sh --timed --shuffle
```
...
```bash
./S05_03_validator.sh <submission_dir> [options]
...
```bash
# Basic validation
./S05_03_validator.sh ~/homework/

# Strict mode before submission
./S05_03_validator.sh ~/homework/ --strict --report
```
