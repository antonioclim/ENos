# 📁 Bash Utilities — SEM06

> **Location:** `SEM06/scripts/bash/`  
...
|--------|---------|----------------|
| `S06_01_setup_seminar.sh` | Install dependencies, create workspace | Yes (first run) |
| `S06_02_interactive_quiz.sh` | CLI quiz with instant feedback | No |
| `S06_03_validator.sh` | Validate homework submissions | No |
...
./S06_02_interactive_quiz.sh

# Validate your homework
./S06_03_validator.sh ~/my_homework/
```
...
```bash
./S06_01_setup_seminar.sh [options]
...
- Workspace directories (`~/os_seminar_sem06/`)
...
```bash
./S06_02_interactive_quiz.sh [options]
...
./S06_02_interactive_quiz.sh --timed --shuffle
```
...
```bash
./S06_03_validator.sh <submission_dir> [options]
...
```bash
# Basic validation
./S06_03_validator.sh ~/homework/

# Strict mode before submission
./S06_03_validator.sh ~/homework/ --strict --report
```
