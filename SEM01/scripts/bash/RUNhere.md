# 📁 Bash Utilities — SEM01

> **Location:** `SEM01/scripts/bash/`  
...
|--------|---------|----------------|
| `S01_01_setup_seminar.sh` | Install dependencies, create workspace | Yes (first run) |
| `S01_02_interactive_quiz.sh` | CLI quiz with instant feedback | No |
| `S01_03_validator.sh` | Validate homework submissions | No |
...
```bash
./S01_01_setup_seminar.sh [options]
...
- Workspace directories (`~/os_seminar_sem01/`)
...
```bash
./S01_02_interactive_quiz.sh [options]
...
```bash
./S01_03_validator.sh <submission_dir> [options]
